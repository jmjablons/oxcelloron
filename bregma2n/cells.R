...
if(temp$data_classifier %in% "neuropil"){temp$what <- "neuropil"}

switch(temp$data_source, 
       abba={
         cat("\n","Combine subregions? y/n")
         temp$divide_subregions <- readline()
         if(temp$divide_subregions %in% "y"){
           d_dat$structure <- val$structures[6]}
...
d_dat = d_dat %>%
  mutate(structure = 
           ifelse(structure %in% c("Cg1","Cg2"), "Cg", structure))
...
temp$dir_subpath <- paste(path_result)
